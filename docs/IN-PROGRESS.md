...
## [알려진 한계, 대응 안 함] Google Docs 모바일뷰 미지원
웹슬롯 모바일뷰 기능(UA 전환)이 Google Docs에서는 작동하지 않음. Google이
모바일 UA 접속 시 로그인 세션 유효 여부와 무관하게 workspace.google.com
마케팅 페이지나 앱 다운로드 유도 화면으로 강제 전환시키는 정책 때문 —
우리 쪽 UA/쿠키/세션 처리와 무관한 외부 정책. Android Dex 모드 등 실사용
환경에서도 동일 현상 확인됨. 사이트별 버튼 숨김 처리는 보류 상태.

## [보류] 즐겨찾기 폴더 드래그앤드롭
"미분류" 카드에서 폴더로 드래그해서 옮기는 기능. 지금은 카드 안 `<select>`
드롭다운으로 폴더 이동 가능(3단계-B1). 즐겨찾기 UI 디자인을 나중에 다시
다듬을 때 드래그앤드롭 붙이는 걸 같이 고려하기로 함(경량 UX 개선이라 급하지
않음).

## [해결됨(외부 원인, 우리 쪽 작업 종료)] Gemini 워크스테이션 재오픈 시 대화방 복원 실패

**결론**: 2026-07-01 세션에서 Google 자체 버그로 최종 확정. 자세한 진단 과정과
근거는 `docs/CHANGELOG-DEV.md`의 "2026-07-01 — Gemini 워크스테이션 재오픈 복원
실패, 원인을 Google 자체 버그로 최종 확정" 항목 참고. 이 문서(IN-PROGRESS)에
있던 기존 진단 기록(2026-06-30 세션까지의 1~4단계 diagnosis, "아직 시도 안 한
것들" 목록 포함)은 전부 CHANGELOG로 이전 완료, 여기서는 삭제함.

**한 줄 요약**: 실패 상태의 Gemini Gem 딥링크 URL을 (1) 우리 앱, (2) 순정 Chrome
주소창, (3) Chrome 자체 탭그룹 — 세 환경 모두에서 동일하게 재현됨을 확인.
Electron/webview/UA/우리 코드 구조와 무관한 Google 1st-party 버그로 확정.

**다시 손대지 말 것**: UA 스푸핑, cold-load 워밍업, selector 기반 렌더링 판정,
텍스트 길이 임계값+재시도 로직 전부 검토했으나 "우리 쪽에서 고칠 문제가
아니다"라는 결론과 함께 전부 폐기(적용 안 함). Google이 자체 수정하기 전까지
재개하지 않음. 관련 상세 근거는 메모리(memory_user_edits)에도 기록됨.
